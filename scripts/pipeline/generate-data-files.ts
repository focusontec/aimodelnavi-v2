...
`;
  ...
  const analysesKo = db.prepare(
    `SELECT ma.*, m.slug FROM model_analyses ma JOIN models m ON m.id = ma.model_id WHERE ma.language = 'ko'`
  ).all() as any[];
  ...
  const analysisKoMap: Record<string, any> = {};
  for (const a of analysesKo) analysisKoMap[a.slug] = buildAnalysisEntry(a);
...
export const modelAnalysesKo: Record<string, ModelAnalysis> = ${JSON.stringify(analysisKoMap, null, 2)};

export function getModelAnalysis(slug: string, locale: string = 'ja'): ModelAnalysis | undefined {
  if (locale === 'ko') return modelAnalysesKo[slug] || modelAnalysesEn[slug] || modelAnalysesJa[slug];
