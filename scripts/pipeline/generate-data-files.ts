...
`;
  ...
  ).all() as { model_id: number; slug: string; summary: string; performance: string; comparisons: string; community: string; use_case_deep: string; latest_news: string; sources_json: string; generated_at: string }[];
...
export const modelAnalyses: Record<string, ModelAnalysis> = ${JSON.stringify(analysisMap, null, 2)};

export function getModelAnalysis(slug: string): ModelAnalysis | undefined {
  return modelAnalyses[slug];
}
`;
