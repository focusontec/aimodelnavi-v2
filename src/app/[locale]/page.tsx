...
import { BarChart3, Calculator, Coins, Search, ArrowRight, TrendingUp, BookOpen, LayoutGrid, ArrowLeftRight, FileText, Sparkles, Zap } from "lucide-react";
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-primary-600 via-primary-700 to-accent-600">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-28 relative">
          <div className="max-w-3xl">
            ...
            <p className="mt-5 text-lg text-primary-100 max-w-2xl leading-relaxed">{t.heroSub}</p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href={`/${locale === "ja" ? "" : locale + "/"}leaderboard`} className="inline-flex items-center gap-2 px-6 py-3.5 bg-white text-primary-700 font-semibold rounded-xl hover:bg-primary-50 transition-colors shadow-lg">
                <BarChart3 className="w-4 h-4" />{t.viewRankings}<ArrowRight className="w-4 h-4" />
              </Link>
              <Link href={`/${locale === "ja" ? "" : locale + "/"}pricing`} className="inline-flex items-center gap-2 px-6 py-3.5 bg-white/10 text-white font-semibold rounded-xl hover:bg-white/20 transition-colors backdrop-blur-sm border border-white/20">
                <Coins className="w-4 h-4" />{t.comparePricing}
              </Link>
            </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-20">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {[{ step: "01", title: t.step1Title, desc: t.step1Desc, icon: Search }, { step: "02", title: t.step2Title, desc: t.step2Desc, icon: BarChart3 }, { step: "03", title: t.step3Title, desc: t.step3Desc, icon: Calculator }].map((s) => {
            const Icon = s.icon;
            return (
              <div key={s.step} className="relative p-6 bg-white border border-gray-200 rounded-xl hover:border-primary-200 hover:shadow-md transition-all">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {sortedPosts.map((post: any) => (
                <Link key={post.slug} href={`/${locale === "ja" ? "" : locale + "/"}blog/${post.slug}`} className="group p-6 bg-white border border-gray-200 rounded-xl hover:border-primary-300 hover:shadow-md transition-all">
                  <div className="flex items-center gap-2 mb-3">
                    <span className="px-2.5 py-0.5 bg-primary-50 text-primary-700 text-xs font-medium rounded-full">{post.tag}</span>
                    <span className="text-xs text-gray-400">{post.date}</span>
                  </div>
                  <h3 className="font-semibold text-gray-900 group-hover:text-primary-600 transition-colors line-clamp-2 mb-2">{locale === "en" ? (post.title_en || post.title) : post.title}</h3>
                  <p className="text-sm text-gray-500 line-clamp-3">{locale === "en" ? (post.excerpt_en || post.excerpt || "") : (post.excerpt || "")}</p>
                </Link>
              ))}
            </div>
          </div>
